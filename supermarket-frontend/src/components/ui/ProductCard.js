    ...
    <div className="card h-100 shadow-sm">
      <Link to={`/products/${product.id}`} className="text-decoration-none">
        <div className="position-relative" style={{ paddingBottom: '56.25%' }}>
          ...
          />
          {product.discount > 0 && (
            <div className="position-absolute badge" style={{ top: '0.5rem', right: '0.5rem', backgroundColor: '#FF5722', color: 'white' }}>
      ...
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-start">
          ...
          <div className="d-flex align-items-center">
            <span className="me-1" style={{ color: '#FFB74D' }}>
        ...
        <div className="mt-3 d-flex justify-content-between align-items-center">
          <div className="d-flex align-items-baseline">
            <span className="fs-5 fw-bold" style={{ color: '#FF8C00' }}>${product.price.toFixed(2)}</span>
            {product.oldPrice && (
              <span className="small text-muted text-decoration-line-through ms-2">${product.oldPrice.toFixed(2)}</span>
            )}
          </div>
          <button className="btn btn-sm rounded-pill" style={{ backgroundColor: '#FF8C00', color: 'white' }}>
            Add to Cart
          </button>
        </div>
      </div>
